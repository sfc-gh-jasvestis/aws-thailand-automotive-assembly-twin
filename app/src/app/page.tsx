...
export default function HomePage() {
  ...
  const executiveCockpit = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard title="Assembly Line OEE" value={kpiVal('Assembly Line OEE', '87.3%')} status="neutral" />
        <KPICard title="Digital Twin Sync" value={kpiVal('Digital Twin Sync', '1.2s')} status="neutral" />
        <KPICard title="Quality Defects (Today)" value={kpiVal('Quality Defects (Today)', '4')} status="warning" />
        <KPICard title="Units Produced (MTD)" value={kpiVal('Units Produced (MTD)', '12,847')} status="neutral" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 grid-cols-1">
        <Chart
          ...
          type="line"
          xKey="period"
          yKeys={[{ key: 'value', name: 'OEE %' }]}
          title="OEE Trend by Line (Daily)"
        />
        <Chart
  ...
  const domainTab1 = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <KPICard title="Cycle Time Variance" value={kpiVal('Cycle Time Variance', '±2.1s')} />
        <KPICard title="Energy per Unit" value={kpiVal('Energy per Unit', '48 kWh')} />
        <KPICard title="Simulation Accuracy" value={kpiVal('Simulation Accuracy', '99.2%')} />
      </div>
      <Chart
        data={data?.detail || [{ x: 'Mon', y: 24 }, { x: 'Tue', y: 28 }, { x: 'Wed', y: 22 }, { x: 'Thu', y: 31 }, { x: 'Fri', y: 26 }, { x: 'Sat', y: 19 }, { x: 'Sun', y: 23 }]}
        type="area"
        xKey="x"
        yKeys={[{ key: 'y', name: 'Units/Hour' }]}
        title="Predicted vs Actual Throughput"
        height={400}
      />
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Chart
          ...
          type="pie"
          xKey="label"
          yKeys={[{ key: 'value', name: 'Defects/1000' }]}
          title="Defect Rate by Shift"
        />
        <ActionMemo
          persona={{ name: 'Kittisak Wongchai', role: 'Plant Manager' }}
          context={{}}
          onGenerate={async () => {
            const memos = [
              {
                ...
                actions: ['Investigate Station 7 weld quality drift', 'Schedule Line-B maintenance (bearing wear)', 'Optimize paint booth airflow via twin sim'],
              },
              {
  ...
  const askAiTab = (
    <div className="h-[600px]">
      <AskAI
        title="Ask AI"
        sampleQuestions={[
          'Which station has the highest defect rate?',
          'Show OEE trend for Line-A vs Line-B this month',
          'What does the digital twin predict for next week?',
        ]}
        mode="sql"
  ...
  const tabs = [
    ...
    { id: 'domain-1', label: 'Digital Twin', icon: '📈', content: domainTab1 },
    { id: 'domain-2', label: 'Quality Control', icon: '⚡', content: domainTab2 },
